# The genera picked for researching further information are taken from the 
# new_overview_by_genus. I.e. they're the genera with more hits than only 
# singletons. Info is taken from wikipedia hits or when necessary, googling.
...
genus_info <- read_excel('input/genus_lookup.xlsx')
...
### Filter out relevant genera -------------
# genera_to_keep <- genus_info$genus[!genus_info$description %in%
                      # c('coral', 'sponge', 'millepora', 'anemone')]

genera_to_keep <- genus_info$genus[genus_info$description == 'fungus']

its1_raw <- its1_raw %>% 
  mutate(genus = word(header, end = 1))

its1_filtered <- its1_raw[its1_raw$genus %in% genera_to_keep, ]

its2_raw <- its2_raw %>% 
  mutate(genus = word(header, end = 1))

its2_filtered <- its2_raw[its2_raw$genus %in% genera_to_keep, ]
...
ggplot(plot_data3, aes(x = PC1, y = PC2, colour = diseased)) +
  ...
  ggtitle(paste0("PCA for ITS2 & original data\n", 
                 round((rel_var3[1] + rel_var3[2]) * 100, 2), 
                 " % of total variation captured")) +
  guides(colour = guide_legend(title = NULL)) +
  scale_colour_manual(values = c("#0072B2", "#D55E00"),
                      labels = c("healthy", "sick")) +
